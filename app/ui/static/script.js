...
function updateActiveTests() {
  ...
  const activeTests = testData.tests.filter(
    (test) => test.status === "running" || test.status === "pending"
  );

  if (activeTests.length === 0) {
    container.innerHTML =
      '<div class="text-center text-muted py-4">No active tests running</div>';
    return;
  }

  container.innerHTML = activeTests
    .map(
      (test) => `
                <div class="test-item">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        ...
                        <div class="d-flex gap-2">
                            <button class="btn btn-sm btn-outline-light" onclick="viewTestDetails(${
                              test.id
                            })">
...
function updateScriptsList() {
  ...
  container.innerHTML = testData.scripts
    .map(
      (script) => `
                <div class="test-item">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <small class="text-muted">Created: ${
                              script.created
                            } • Size: ${script.size}</small>
                        </div>
                        <div class="d-flex gap-2">
                            <span class="status-badge status-${
                              script.status === "active" ? "passed" : "pending"
                            }">
                                ${script.status}
                            </span>
                            <div class="dropdown">
                            ...
                            ${testData.tests
                              .map(
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <button class="btn btn-sm btn-outline-light me-1" onclick="viewTestReport(${
                                          test.id
                                        })">
                                            <i class="fas fa-eye"></i>
                                        </button>
                                        <button class="btn btn-sm btn-outline-light" onclick="rerunTest(${
                                          test.id
                                        })">
...
function startSystemLog() {
  ...
  const logMessages = [
    ...
    {
      type: "success",
      message: "Generated new test script: test_navigation.py",
    },
  ];
...
function generateTestScript() {
  ...
  const originalText = btn ? btn.html : "Generate Test Script";
...
function runScript() {
  const testName =
    document.getElementById("testName").value || "Generated Test";
...
setInterval(() => {
  const runningTests = testData.tests.filter(
